...
def rela():	
	...
	if "INFORMATION GATHERING" in tsfeitos:
		...
		if robots == False:
			...
			pdf.ln()
			pdf.image('robots.png',w= 200, h=190)
			pdf.ln()
		pdf.set_font('DejaVu', '', 20)
		pdf.ln()
	...
	if "CONFIGURATION AND DEPLOY MANAGEMENT TESTING" in tsfeitos:
		...
		pdf.ln()
		pdf.cell(40,10,"(CONFIG-007) Verificar uso de Strict-Transport-Security: ")
		pdf.ln()
		pdf.set_font('DejaVu','',10)
		if strict == True:
			pdf.cell(40,10,"Ausência de cabeçalho Strict-Transport-Security!")
		elif strict == False:
			pdf.cell(40,10,"Cabeçalho Strict-Transport-Security habilitado!")
		pdf.ln()
		pdf.set_font('DejaVu','',20)
		pdf.cell(40,10,"(CONFIG-008) Identificar política de domínio exposta: ")
		pdf.ln()
		pdf.set_font('DejaVu','',10)
		if cross == True:
			pdf.cell(40,10,"Arquivo de política exposto:")
			pdf.ln()
			pdf.cell(40,10,'Arquivo crossdomain.xml:')
			pdf.ln()
			...
			pdf.cell(40,10,"Arquivo de política exposto:")
			pdf.ln()
			pdf.cell(40,10,'Arquivo clientaccesspolicy.xml:')
			pdf.image('clientaccess.png',w= 190, h=100)
			pdf.ln()
		else:
			pdf.cell(40,10,"Não foram identificados arquivos de política expostos")
			pdf.ln()
...
def authz():
	...
	murl = url+teste
	print "     [-] - GET "+murl
	req1 = requests.get(url=murl, timeout=10)
...
def conf():
	...
	print len(diret)," diretórios descobertos."
	if len(diret) == 42:
...
def info():
	...
	req1 = requests.get(url=url+'/robots.txt', timeout=10)
	if req1.status_code == 200:
